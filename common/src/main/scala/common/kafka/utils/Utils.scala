...
object Utils extends LazyLogging {
  ...
  def setupKafkaConsumers(kafkaConfig: Config): KafkaConsumers = {
    val brokers = sys.env.getOrElse("KAFKA_BROKERS", kafkaConfig.getString("brokers"))
    val schemaRegistryUrl = sys.env.getOrElse("SCHEMA_REGISTRY_URL", kafkaConfig.getString("schemaRegistryUrl"))

    logger.info("Setting up Kafka producers with brokers: " + brokers + " and schema registry URL: " + schemaRegistryUrl)
    KafkaConsumers(
      initEventConsumer = KafkaConsumerUtils.createAvroConsumer[InitEvent](brokers, schemaRegistryUrl),
      matchEventConsumer = KafkaConsumerUtils.createAvroConsumer[MatchEvent](brokers, schemaRegistryUrl),
      inAppPurchaseEventConsumer = KafkaConsumerUtils.createAvroConsumer[InAppPurchaseEvent](brokers, schemaRegistryUrl),
      globalKtableConsumers = KafkaConsumerUtils.createStringConsumer[String](brokers),
    )
  }
  ...
  def populateGlobalKTableTopics(producer: KafkaProducer[String, String], globalKTableTopics: Config): Unit = {
